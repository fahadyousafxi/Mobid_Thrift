
import 'package:flutter/material.dart';

class AppColors {

  static Color appBarColor = Colors.black;
  static Color appBarTitleColor = Colors.white;
  static Color appBarIconColor = Colors.white;
  static Color myIconColor = Colors.black;
  static Color scaffoldColor = Colors.white;

  static Color drawerIconColor = Colors.white;
  static Color drawerTextColor = Colors.white;
  static Color drawerColor = Colors.black;
  static Color drawerDividerColor = Colors.white38;

  static Color normalTextColor = Colors.black;
  static Color headingTextColor = Colors.black;

  static Color textButtonColor = Colors.blue;
  static Color elevatedButtonColor = Colors.black;
  static Color otherButtonColor = Colors.red;
  static Color buttonColorBlue = Colors.blue;

  static Color appMainColor = Colors.black;
  static Color myBlackColor = Colors.black;
  static Color myWhiteColor = Colors.white;
  static Color myRedColor = Colors.red;
  static Color appSecondColor = Colors.blue;
  static Color appThirdColor = Colors.red;
  static Color appFourthColor = Colors.white;
}